...
namespace BOM
{
    class DateInsert
    {
        public string DateInsert1()
        {
            ...
            int errorCheck = 0;
            for (int i = 0; i < 999; i++) // >> 버튼 한번클릭마다 인트 i = 0 초기값을 올려줘야함
            {
                try
                {
                    errorCheck++;
                    price = 0;
                    prono = new Random().Next(1, 7);
                    ea = new Random().Next(1, 4);
                    year = rd.Next(2016, (int)DateTime.Today.Year);
                    month = rd.Next(1, 12);
                    day = rd.Next(1, 28);
                    ...
                    //////////////////////////////////////////////////////////////////////////////////
                    sqlParameters2[0] = new SqlParameter("@cus_No", new Random().Next(5, 10)); //5~9번 있음
                    sqlParameters2[1] = new SqlParameter("@emp_No", new Random().Next(1, 5)); //4명
                    sqlParameters2[2] = new SqlParameter("@pro_No", prono); //6개
                    sqlParameters2[3] = new SqlParameter("@cus_Order_OrderNo", i); // >> 버튼 한번클릭마다 인트 i = 0 초기값을 올려줘야함
